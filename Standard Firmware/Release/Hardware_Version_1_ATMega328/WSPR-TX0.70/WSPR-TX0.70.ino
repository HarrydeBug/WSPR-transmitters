/*
  ...
  0.70 Added power saving in WSPR Beacon during long TX pauses, puts GPS in power save but leaves Si5351 On to minimize drift issues.
*/

const uint8_t  SoftwareVersion = 0; //0 to 255. 0=Beta
const uint8_t  SoftwareRevision = 70; //0 to 255
...
void setup()
{
  ...
  else //No data was found in EEPROM, set some defaults
  {
    ...
    GadgetData.WSPRData.TXPowerdBm = 13;
    for (int i = 0; i <= 16; i++)
    {
  ...
  if (Product_Model == 1017) //The mini dont have any relays, it's Status LED is on a different pin and it has pin to power the Si5351
  {
    StatusLED = A2;
          ...
          if ((GPSS == 0) && ((GPSM % 2) == 0))//If second is zero at even minute then start WSPR transmission
          {
            ...
            if (LastFreq ())
            {
              if ((GadgetData.TXPause > 60) && (Product_Model == 1017)) //More than a minute of TX delay, turn of the GPS for most of the time to save power (WSPR-TX Mini only)
              {
                ...
                delay (1000UL * (GadgetData.TXPause - 10)); //Do noting until there is 10 seconds left of delay
...
void PowerSaveON()
{
  ...
  { //If its the WSPR-TX Mini it has a Quectel L80-R GPS, send the correct string to it put it to sleep
    ...
    //digitalWrite(SiPower, HIGH); //Dont turn off Si5351 in this version.
  }
  else
  {
