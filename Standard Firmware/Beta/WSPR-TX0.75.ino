/*
  ...
  0.74 Improvment in GUI responsivness when in WSPR Beacon. Additional power saving for the Mini, MCU goes to sleep if TX pause is longer than a minute.
  0.75 Added MCU VCC Voltage info, added support for Pico model
...
const uint8_t  SoftwareRevision = 75; //0 to 255
...
const uint16_t Product_Model                              =1020;
...
#define GPSPower A1    //Sleep-Wake control of the GPS on the WSPR-TX Pico
      ...
      while (gps.available( GPSSerial )) { //If GPS data available - process it
        ...
        if (fix.valid.location && fix.valid.date && fix.valid.time)
        {
          ...
          if ((GPSS == 0) && ((GPSM % 2) == 0))//If second is zero at even minute then start WSPR transmission
          {
            ...
            {
              DoIdle(); //Abort and start ideling as either the Call sign is not set (1) or a serial command interupted the transmission (2).
              return;
            }
