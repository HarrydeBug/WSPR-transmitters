/*
  ...
  0.62 Added functionality to automatically use one of the Low Pass filter in WSPR and SignalGen routines (New rutines - PickLP,FreqToBand)
...
const uint8_t  SoftwareRevision = 63; //0 to 255

// Product model. WSPR-TX_LP1 transmitter    =1011
// Product model. WSPR-TX Desktop transmitter=1012
...
#define Product_Model              1012         // Product number. WSPR Desktop transmitter=1012
...
void setup()
{
  ...
  {
     Serial.println(F("{MIN} ZachTek WSPR LP1 transmitter"));  
  }
  if (Product_Model==1012)
  {
  ...
  else  //No data was found in EEPROM, set some defaults
  {
    ...
    FactoryData.LP_C_BandNum = 2; //Low Pass filter C is 160m
    FactoryData.LP_D_BandNum = 3; //Low Pass filter D is 80m
...
void loop()
{
  ...
  /*
    SetLPFilter ('A');
    Serial.println(F("{MIN} Filter A"));
    DoSerialHandling();
    delay (2000);

    DoSerialHandling();
    SetLPFilter ('B');
    Serial.println(F("{MIN} Filter B"));
    DoSerialHandling();
    delay (2000);
    DoSerialHandling();
...
void SetLPFilter (uint8_t Filter)
{
  if ((FactoryData.HW_Version = 1) && (FactoryData.HW_Revision = 4)) // Early Hardware had different Relay driving electronics
  {
    //De-energize all relays
  ...
  else
  {
    switch (Filter) {
      case LP_A:
        ...
        digitalWrite(Relay2, LOW);
        digitalWrite(Relay3, LOW);
        Serial.println(F("{MIN} LP Filter A in use"));
        break;
      case LP_B:
        digitalWrite(Relay1, HIGH);
        digitalWrite(Relay2, LOW);
        digitalWrite(Relay3, LOW);
        Serial.println(F("{MIN} LP Filter B in Use"));
        break;
      case LP_C:
        digitalWrite(Relay1, LOW);
        digitalWrite(Relay2, LOW);
        digitalWrite(Relay3, HIGH);
        Serial.println(F("{MIN} LP Filter C in Use"));
        break;
      case LP_D:
        digitalWrite(Relay1, LOW);
        digitalWrite(Relay2, HIGH);
        digitalWrite(Relay3, HIGH);
        Serial.println(F("{MIN} LP Filter D in use"));
        break;
    }
  }
}

//Out of the four possible LP filters fitted - find the one that is best for TX on TXBand
void PickLP (uint8_t TXBand)
{
  boolean ExactMatch=false;
  uint8_t LPToUse=LP_A;
/*
  Serial.print(F("{MIN} TXBand="));
  Serial.println(TXBand);
  Serial.print(F("{MIN} Factory LP_A="));
  Serial.println(FactoryData.LP_A_BandNum);
  Serial.print(F("{MIN} Factory LP_B="));
  Serial.println(FactoryData.LP_B_BandNum);
  Serial.print(F("{MIN} Factory LP_C="));
  Serial.println(FactoryData.LP_C_BandNum);
  Serial.print(F("{MIN} Factory LP_D="));
  Serial.println(FactoryData.LP_D_BandNum);
