...
const uint8_t  SoftwareRevision = 74; //0 to 255
...
void setup()
{
  ...
  switch (Product_Model) {
    ...
    case 1017:
      Serial.println(F("{MIN} ZachTek WSPR Mini transmitter, Power Save version"));
      break;
    ...
    while (!Serial.available()) { //Do until incoming serial command
      while (gps.available( GPSSerial )) { //If GPS data available - process it
        ...
        {  //Waitng for GPS location fix
          ...
          LEDBlink(); //singelblink to indicate waiting for GPS Lock
...
void GPSGoToSleep()
{
  ...
  { //If its the WSPR-TX Mini it has a Quectel L80-R GPS, send the correct string to it put it to sleep
    GPSSerial.println(" "); //Send something to clear out any junk in the serial buffer.
    delay(800);
