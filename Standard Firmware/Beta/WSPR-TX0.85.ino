/*
  ...
  0.85 Picos combined 20m and 30m lowpass filter correctly reported to PC program, An unresponsive GPS will be reset after some time
...
const uint8_t  SoftwareRevision = 85; //0 to 255
 ...
 //Read all the Factory data from EEPROM at position 400
  ...
  else  //No factory data was found in EEPROM, set some defaults
  {
   ...
   /*
    ...
    {
      FactoryData.HW_Revision = 3; // Hardware revision
    ...
    if (InputCMD[1] == 'F') {
      ...
      if ((InputCMD[2] == 'L') && (InputCMD[3] == 'P')) {
        ...
        else //Get Option
        {
          ...
          //The same goes for the Pico regardless of hardware version
          //The PC will indicate these bands with the little green square in the GUI
          if ((FactoryData.HW_Version == 1) & (FactoryData.HW_Revision > 9) || (Product_Model == 1028) )
          {  
          ...
          if ((GPSS == 0) && ((GPSM % 2) == 0))//If second is zero at even minute then start WSPR transmission
          {
           ...
           if (GadgetData.WSPRData.PowerOption == Altitude)// If Power field should be used for Altitude coding  
           {
            ...
            if (LastFreq ())  //If all bands have been transmitted on then pause for user defined time and after that start over on the first band again
            {
              ...
              { //Regular pause if we did not go to sleep then do a regular pause and send updates to teh GUI for the duration
