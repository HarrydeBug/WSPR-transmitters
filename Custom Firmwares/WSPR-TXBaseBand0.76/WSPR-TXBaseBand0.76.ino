...
/*
  ...
  For Arduino Pro Mini ATMega 328 8MHz, 3.3V
  ...
  0.76 Added Support for WSPR-TX Pico, GPS position updates in idle and signal gen mode, Pico status LED fast blink during TX instead of steady lit (save power, the Pico LED is very bright)
       The Pico will always boot in to WSPR Beacon regardless of Boot configuration, this is a failsafe. Moved check if Call Sign is set from SendWSPRBlock() to DoWSPR()
  BASE BAND SPECIAL VERSION 
  This is a special experimental version that requires an external divider on the output.
  It is usefull for generating baseband WSPR e.g 1500Hz for direct input to microphone input on a transmitter or for modulating a laser.
  As the Si5351 can not go as low as 1500Hz the ouput is generated at a higher frequency and the user has to use a digital divider like a 74HC4040 to arrive at 1500Hz
  The division rate is configurable with the constant ExtDivision
  In WSPR Beacon mode the band settings will be ignored and the output frequency will allways be 1500 * ExtDivision +/- the WSPR symbol shift
  The WSPR symbole shift is also multiplied so instead of the normal shift of 1.46Hz per symbol it will be 1.46Hz * ExtDivision
...
//****** CHANGE THIS VALUE TO MATCH YOUR EXTERNAL DIVISION RATE ************
const int ExtDivision = 64;
//**************************************************************************


const uint8_t  SoftwareVersion = 0; //0 to 255. 0=Beta
const uint8_t  SoftwareRevision = 76; //0 to 255
...
void setup()
{
  ...
  Serial.print(F("{MIN} Special BaseBand Firmware version "));
  ...
  Serial.println(F("{MIN} For use with external divider"));
  Serial.print(F("{MIN} Compiled for a division rate of "));
  Serial.println(ExtDivision);
  ...
  else  //No factory data was found in EEPROM, set some defaults
  {
    ...
    if (Product_Model == 1012)    //Desktop Model, set default LP as a Mid version
    {
      FactoryData.LP_A_BandNum = 4; //Low Pass filter A is 40m
      FactoryData.LP_B_BandNum = 5; //Low Pass filter B is 30m
      FactoryData.LP_C_BandNum = 6; //Low Pass filter C is 20m
      FactoryData.LP_D_BandNum = 7; //Low Pass filter D is 17m
    }
...
void loop()
{
  ...
  while (gps.available( GPSSerial )) { //Handle Serial data from the GPS as they arrive
    ...
    if ((GPSS % 4) == 0) //Send some nice-to-have info every 4 seconds, this is a lot of data so we dont want to send it to often to risk choke the Serial output buffer ot the PC configuration software
    {
...
void DoWSPR ()
{
  ...
  else
  {
    ...
    si5351aOutputOff(SI_CLK0_CONTROL);
    SendAPIUpdate (UMesCurrentMode);
    Serial.print(F("{MIN}Starting base band tranmission on 1500Hz x "));
    Serial.print(ExtDivision);
    Serial.print(F("="));
    Serial.print(1500UL * ExtDivision);
    Serial.println(F("Hz"));
      ...
      while (gps.available( GPSSerial )) { //If GPS data available - process it
        ...
        if (fix.valid.location && fix.valid.time)
        {
          ...
          if ((GPSS == 0) && ((GPSM % 2) == 0))//If second is zero at even minute then start WSPR transmission
          {
            ...
            {
              DoIdle(); //Abort and start ideling as a serial command interupted the transmission (1).
              return;
            }
            ...
            {
              if ((GadgetData.TXPause > 60) && (Product_Model == 1017 )) //More than a 60 sec. of TX delay then do power saving (Mini model only)
              {
              ...
              { //Not a long pause and not a Mini so do a regular pause after the tranmission for the legth of time the user has specified in the GUI
...
int SendWSPRBlock()
{
  ...
  for (i = 0; i < 162; i++)  //162 WSPR symbols to transmitt
  {
    ...
    uint64_t tonefreq;
    tonefreq = freq + ((tx_buffer[i] * 146UL * ExtDivision));  //~1.46 Hz  Tone spacing in centiHz
    ...
    {
      if (!blinked)   {  //do short blinks on WSPR TXon Status LED
        for (int i = 0; i < 11; i++)
        {
          digitalWrite(StatusLED, HIGH);
          delay (5);
...
static void smartdelay(unsigned long delay_ms)
{
  ...
  do
  {
    ...
    if ((TimeLeft > 1000)) {
      ...
      //SendSatData(); //Send Satellite information to the PC GUI
      SendAPIUpdate(UMesTime);
    }
  } while ((TimeLeft > 0) && (!Serial.available())) ; //Until time is up or there is serial data received
  if (delay_ms > 1000) Serial.println (F("{MPS} 0"));//Send 0 so it looks neater in the GUI but only for longer delays to conserve Serial data
}
...
void NextFreq (void)
{
  freq = 150000UL * ExtDivision; //frequency in centiHz, 1500Hz * division 
}
...
void GPSWakeUp ()
{
  switch (Product_Model) {
    case 1017:  //Mini
      //Send anythiing on it's serial line to wake it up
      GPSSerial.println(" ");
      //GPSSleep = false;
      delay(100); //Give the GPS some time to wake up
      break;
