...
const uint8_t  SoftwareRevision = 79; //0 to 255
 ...
 //Read all the Factory data from EEPROM at position 400
  ...
  switch (Product_Model) {
    ...
    case 1028:
      ...
      Serial.println(F("{MIN} Altitude coding in PWR field 0-18km"));
      StatusLED = A2; //Status LED uses a different output on the Pico
      //pinMode(SiPower, OUTPUT);
      //digitalWrite(SiPower, LOW);    //Turn on power to the Si5351
      // pinMode(GPSPower, OUTPUT);
      //digitalWrite(GPSPower, HIGH);  //Turn on the GPS
      break;
  }
  ...
  if ((InputCMD[0] == '[') && (InputCMD[4] == ']')) { //A Command,Option or Data input
    ...
    if (InputCMD[1] == 'F') {
      ...
      if ((InputCMD[2] == 'L') && (InputCMD[3] == 'P')) {
        ...
        {
          //If Hardvare is V1 R10 and higher it has a few filters that can do more than one band, indicate by sending out these extra bands so the PC config software
      ...
      while (gps.available( GPSSerial )) { //If GPS data available - process it
        ...
        if (fix.valid.location && fix.valid.time)
        {
          ...
          if ((GPSS == 0) && ((GPSM % 2) == 0))//If second is zero at even minute then start WSPR transmission
          {
            ...
            Serial.print(F("{MIN} Altitude is :"));
            Serial.print(fix.altitude());
            Serial.println("m");
            Serial.print(F("{MIN} PWR set to :"));
            Serial.println(GadgetData.WSPRData.TXPowerdBm);
            ...
            if (SendWSPRBlock () != 0) //Send a WSPR "Packet" for 1 minute and 50 seconds
            {
              ...
              //DoIdle(); //Abort and start ideling, in the idle loop all Serial commands will be properly handled.
              return;
            }
...
int SendWSPRBlock()
{
  ...
  boolean blinked;
    ...
    for (i = 0; i < 162; i++)  //162 WSPR symbols to transmitt
    {
      ...
      {
        if (!blinked)   {  //do pulsing blinks on Status LED to indicate WSPR Beacon transmission 
          for (int i = 0; i < 5; i++)
          {
            digitalWrite(StatusLED, HIGH);
            delay (10);
...
static void smartdelay(unsigned long delay_ms)
{
  ...
  if (delay_ms > 1000) Serial.println (F("{MPS} 0"));//When pause is complete send Pause 0 to the GUI so it looks neater. But only if it was at least a one second delay 
}
...
void MCUGoToSleep( int SleepTime)//Sleep time in seconds, accurate to the nearest 8 seconds
{
  ...
  for (int i = 0; i < SleepLoop; i++)
  {
    ...
    //Just woke upp after 8 sendos of sleep, do a short blink to indicate that I'm still running 
    digitalWrite(StatusLED, HIGH);
    delay (30);
    digitalWrite(StatusLED, LOW);
  }
